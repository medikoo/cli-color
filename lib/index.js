...
module.exports = exports = defineProperties(function (msg) {
	...
	up: d('cew', function (num) {
		return '\x1b[' + (isNaN(num) ? 0 : max(floor(num), 0)) + 'A';
	}),
	down: d('cew', function (num) {
		return '\x1b[' + (isNaN(num) ? 0 : max(floor(num), 0)) + 'B';
	}),
	right: d('cew', function (num) {
		return '\x1b[' + (isNaN(num) ? 0 : max(floor(num), 0)) + 'C';
	}),
	left: d('cew', function (num) {
		return '\x1b[' + (isNaN(num) ? 0 : max(floor(num), 0)) + 'D';
	}),
	move: d('cew', function (x, y) {
		x = isNaN(x) ? 0 : floor(x);
		y = isNaN(y) ? 0 : floor(y);
		if (x) {
			x = (x > 0) ? exports.right(x) : exports.left(-x);
		} else {
			x = '';
		}
		if (y) {
			y = (y > 0) ? exports.down(y) : exports.up(-y);
		} else {
			y = '';
		}
		return x + y;
